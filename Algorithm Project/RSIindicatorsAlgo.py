...
class IndicatorSuiteAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.symbol = "SPY"
        self.customSymbol = "VIX"
        self.price = None

        self.SetStartDate(2019, 1, 1)  #Set Start Date
        self.SetEndDate(2020, 5, 20)    #Set End Date
        ...
        vixClose = Identity(self.customSymbol)

        # this will create a new indicator whose value is FB/SPY
        self.ratio = IndicatorExtensions.Over(vixClose, spyClose)
    ...
    def OnData(self, data):
        ...
        if not self.Portfolio.HoldStock:
            ...
            self.Debug('Purchased VIX on ' + self.Time.strftime('%Y-%m-%d'))
    ...
    def update_plots(self):
        ...
        # Custom data indicator
        self.Plot('RSI-VIX', self.rsiCustom)
 ...
 # Set Limit Order to be good until market close
    ...
    self.MarketOrder("VIX", 315, lastClose * decimal.Decimal(.999))

close = self.Securities["VIX"].Close
stopPrice = close * .30 # Trigger stop limit when price falls 1%.
limitPrice = close * 70.0 # Sell equal or better than 70% > close.
stopLimitTicket = self.StopLimitOrder("VIX", stopPrice, limitPrice)
